...
public class UFOClient implements Client {
	@RestService
	org.biu.ufo.control.rest.internal.ufoserver.Client _client;
	
	@Override
	public List<Station> getStations(String latitude, String longitude, float distance) {
		List<org.biu.ufo.control.rest.internal.ufoserver.Station> stations = _client.getStations(latitude, longitude, distance);
		
		List<Station> result = new ArrayList<Station>(stations.size());
		for(org.biu.ufo.control.rest.internal.ufoserver.Station ufoStation : stations) {
