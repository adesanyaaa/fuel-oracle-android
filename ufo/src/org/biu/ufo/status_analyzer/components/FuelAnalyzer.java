package org.biu.ufo.status_analyzer.components;
...
import org.biu.ufo.status_analyzer.StatusSharedPref_;
import org.biu.ufo.status_analyzer.commands.FuelLossProcessEndedStatusMessage;
import org.biu.ufo.status_analyzer.commands.FuelLossProcessStartedStatusMessage;
...
public class FuelAnalyzer {
	...
	public void onFuelLevelUpdate(FuelLevelMessage message){
		if (firstTimeInit){
			firstTimeInit = false;
			refFuelLevel= (float) message.fuelLevelValue;
			
		}else{
			currentFuelLevel = (float) message.fuelLevelValue;
			fuelDeltaCheck();
		}
